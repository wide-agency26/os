...
export default function TaskTypeListView() {
  ...
  const handleBulkDelete = async () => {
    if (!confirm(`Delete ${selectedIds.size} task type(s)?`)) return;
    setIsDeleting(true);
    const supabase = createClient();
    await (supabase as any).from("erp_task_types").delete().in("id", Array.from(selectedIds));
    ...
    <Workspace>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/app/projects/task-type/new"
        ...
        <table className="w-full text-left text-[13px]">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={4} className="p-8 text-center text-gray-500">No task types found.</td></tr>
            ) : items.map((item, idx) => (
              <tr key={item.id || idx} onClick={() => router.push(`/app/projects/task-type/${item.id}`)}
