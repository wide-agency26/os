...
export default function ActivityCostListView() {
  ...
  const fetchItems = async () => {
    ...
    const { data } = await (supabase as any)
      .from("erp_activity_costs")
      .select(`
        id, costing_rate, billing_rate,
  ...
  const handleBulkDelete = async () => {
    if (!confirm(`Delete ${selectedIds.size} activity cost(s)?`)) return;
    setIsDeleting(true);
    const supabase = createClient();
    await (supabase as any).from("erp_activity_costs").delete().in("id", Array.from(selectedIds));
  ...
  return (
    <Workspace>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Activity Costs</h2>
        <div className="flex items-center gap-3">
          ...
          <Link href="/app/projects/activity-cost/new"
        ...
        <table className="w-full text-left text-[13px]">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-4 py-3 font-medium text-gray-600">Costing Rate</th>
              <th className="px-4 py-3 font-medium text-gray-600">Billing Rate</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="p-8 text-center text-gray-500">No activity costs found.</td></tr>
            ) : items.map((item, idx) => (
              <tr key={item.id || idx} onClick={() => router.push(`/app/projects/activity-cost/${item.id}`)}
                ...
                <td className="px-4 py-3 font-medium text-gray-900">{item.person?.full_name || "-"}</td>
                <td className="px-4 py-3 text-gray-600">{item.activity_type?.name || "-"}</td>
                <td className="px-4 py-3 text-gray-600">{formatCurrency(item.costing_rate)}</td>
                <td className="px-4 py-3 text-gray-600">{formatCurrency(item.billing_rate)}</td>
