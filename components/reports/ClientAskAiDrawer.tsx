  ...
  return (
    <div className="fixed inset-0 z-[80] flex justify-end no-print">
      <button
        ...
        onClick={onClose}
      />
      <aside className="relative w-full max-w-md h-full bg-surface shadow-xl border-l border-border flex flex-col animate-in slide-in-from-right pb-[var(--os-bottom-nav)]">
        <header className="px-4 py-3 border-b border-border flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-text-primary">
              <MessageSquare size={16} />
              <h2 className="text-[15px] font-semibold">Ask about this report</h2>
            </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-md text-text-muted hover:bg-surface-raised min-h-11 min-w-11 sm:min-h-0 sm:min-w-0 inline-flex items-center justify-center"
          >
            <X size={16} />
          </button>
        </header>
              ...
              <div className="flex flex-col gap-2">
                {SUGGESTIONS.map((s) => (
                  <button
                    ...
                    type="button"
                    onClick={() => void ask(s)}
                    className="text-left text-[13px] px-3 py-2.5 min-h-11 rounded-lg border border-border bg-surface-raised hover:bg-background text-text-primary"
                  >
                    {s}
                  </button>
                ))}
            ...
            <div
              ...
              className={`rounded-lg px-3 py-2.5 text-[13px] leading-relaxed ${
                m.role === "user"
                  ...
                  : "bg-surface-raised border border-border text-text-primary mr-2"
              }`}
            >
        ...
        <footer className="p-3 border-t border-gray-200">
          ...
          >
            <input
              ...
              className="flex-1 border border-border rounded-lg px-3 py-2 min-h-11 text-[13px] outline-none focus:ring-1 focus:ring-accent bg-surface"
              disabled={loading}
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="px-3 py-2 min-h-11 rounded-md bg-accent text-white disabled:opacity-50"
            >
              <Send size={15} />
            </button>
          </form>
