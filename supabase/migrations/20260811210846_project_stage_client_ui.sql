-- Product language: Prospect / Lead / Client (signed kept as alias → migrated to client).
...
ALTER TABLE public.projects
  ...
  CHECK (stage IS NULL OR stage IN ('prospect', 'lead', 'client', 'signed', 'completed'));

COMMENT ON COLUMN public.projects.stage IS
  'Accounting pillar: prospect→unidentified, lead→identified, client/signed/completed→actual.';
